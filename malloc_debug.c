...
void* malloc(size_t size) {
  ...
  void *real_malloc_result = real_malloc(size);

  if (real_malloc_result != NULL) {
    ...
    // Copy size's string representation to the current end of info, which 
    // should be just the end of the prefix
    // Also need the size of the entire info string, so that we can write info 
    // to stderr without truncation or trailing garbage characters
    int info_str_size = 
      info_prefix_length + 
    ...
    // instead of this overriding one, but write works and is simpler to 
    // implement 
    write(STDERR_FILENO, info, (size_t)info_str_size);
  }

  return real_malloc_result;
}
